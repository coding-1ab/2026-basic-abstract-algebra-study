...
#section(level: lv-easy)[
  #question[
    ...
    #proof[
      ...
      As there exists an element $a = 7$ that is an element of $ZZ$ but not an element of $10 ZZ + 8 ZZ$, the condition $a in ZZ -> a in 10 ZZ + 8 ZZ$ is false. Thus, $ZZ subset.not 10 ZZ + 8 ZZ$. As there is a counterexample in a case where $n = 10$ and $m = 8$, the statement is false.
    ]
  ]
]
...
#section(level: lv-hard)[
  #question[
    ...
    In fact, any two coprime integers $n$ and $m$ have the same property, i.e., $n perp m -> n ZZ + m ZZ = ZZ$ for integers $n$, $m$. Moreover, not only in $ZZ$, but in any _principal ideal domain_, the _ideal_ generated by two coprime elements is the entire ring.
  ]
]

#section(level: lv-hard)[
  #question[
    Show using @def:subset that for any two positive integers $n$ and $m$, $mu_n subset mu_(n m)$.
...
#section(level: lv-levi)[
  ...
  #question[
    Prove or disprove that $A triangle B subset C$ if and only if $A union B subset (A inter B) union C$.
  ][
    #truestmt
...
#section(level: lv-levi)[
  ...
  #long-answer[
    ...
    If you have learned set theory, you might have immediately realized that $T$ is uncountable while $mu_oo$ is countable.

    Here goes the proof!

    #proof[
      Let's first ensure that $mu_oo$ is countable. We know that each $mu_n$ is finite as $|mu_n| = n$ and not empty as they always include $1$, making $mu_oo$ not empty. Now take any function $f : ZZ^+ times ZZ^+ -> mu_oo$ such that if $m <= n$, then $f(n, m)$ is the $m$-th element of $mu_n$ when ordered by arguments. As such $f$ always exists and is clearly surjective, we have $|mu_oo| <= |ZZ^+ times ZZ^+| = aleph_0$. Hence, $mu_oo$ is countable. On the other hand, $T$ is uncountable as the argument $arg : T -> [0, 2 pi)$ is bijective, and $[0, 2 pi)$ is uncountable. Since one is countable while the other is uncountable, they cannot be equal.
    ]
    ...
    #proof[
      ...
      Since $5^n$ is a real number, its imaginary part, $B_n$, must be $0$ when $k = n$. However, the congruence equation $3^n equiv 0 thick (mod 5)$ does not have a solution, as $3 perp 5$. Thus, such positive integer $n$ does not exist, contradicting with the assumption. Therefore, $x$ cannot be an $n$-th root of unity for any positive integer $n$.
    ]
