...
#title-slide[
  ...
  #datetime(year: 2026, month: 5, day: 30).display("[month repr:long] [day], [year]")
]
...
Notation: $(X, ast.op.o)$, $x ast.op.o y$

== Examples of binary operations

Well-defined or not?

+ Positive integers, addition. $(ZZ^+, +)$
+ Integers, addition. $(ZZ, +)$
+ Real numbers, addition. $(RR, +)$
+ Integers, subtraction. $(ZZ, -)$
+ Positive integers, subtraction. $(ZZ^+, -)$
+ Integers, multiplication. $(ZZ, times)$
+ Real numbers, division. $(RR, slash)$
+ Real numbers, exponentiation. $(RR, hat)$

#pagebreak()

+ Real numbers, larger one between two numbers. $(RR, max)$
+ Integers, greatest common divisor. $(ZZ, gcd)$
+ Integers, remainder. $(ZZ, mod)$
+ Strings, concatenation. $(Sigma^*, +)$
+ Subsets of a set $X$, intersection. $(cal(P)(X), inter)$
+ Subsets of a set $X$, Cartesian product. $(cal(P)(X), times)$
+ Matrices, multiplication. $("GL"_RR(n), times)$
+ Three-dimensional Euclidean vector space, inner product. $(RR^3, dot)$
+ $n$-th roots of unity, multiplication. $(mu_n, times)$

== Identity element[항등원]

$x ast.op.o e = x$ and $e ast.op.o x = x$
